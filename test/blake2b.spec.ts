...
function bufferResponse(response: String[]) {
  return Uint8Array.from(
    Buffer.concat(
      response.map((x: String) => Buffer.from(x.substring(2), "hex"))
    )
  );
}
...
describe("Blake2b hash", function () {
  ...
  it("hash two entries", async () => {
    ...
    const response = bufferResponse(
      await blake2bContract.blake2b([input0, input1])
    );
    expect(response).to.deep.equal(answer);

    const response2 = bufferResponse(
      await blake2bContract.blake2bPair([input0, input1])
    );
  ...
  it("hash three entries", async () => {
    ...
    const answer = bufferAnswer(
      await blake2b(Buffer.concat([input0, input1, input2]))
    );
    const response = bufferResponse(
      await blake2bContract.blake2b([input0, input1, input2])
    );
    expect(response).to.deep.equal(answer);

    const response2 = bufferResponse(
      await blake2bContract.blake2bTriple([input0, input1, input2])
    );
    expect(response2).to.deep.equal(answer);
  });
  ...
  it("hash five entries", async () => {
    ...
    const answer = bufferAnswer(
      await blake2b(Buffer.concat([input0, input1, input2, input0, input1]))
    );
    const response = bufferResponse(
      await blake2bContract.blake2b([input0, input1, input2, input0, input1])
    );
    expect(response).to.deep.equal(answer);

    const response2 = bufferResponse(
      await blake2bContract.blake2bFive([
        input0,
        input1,
        input2,
        input0,
